function laodingFunc(isLoading, loading) {
  if (isLoading) {
    loading.style.display = "grid";
    Array.from({ length: 4 }).map(() => {
      let divLoading = document.createElement("div");
      divLoading.innerHTML = `
          <div role="status" class="max-w-sm rounded-lg p-4">
          <div
            class="animate-pulse w-full bg-gray-300 h-[350px] rounded-lg mb-5 flex justify-center items-center"
          >
            <svg
